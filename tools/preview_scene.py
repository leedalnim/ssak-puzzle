...
def render(stage, progress, hero_cell, scale_view=3):
    ...
    # 배경 그라데이션
    top = hexlerp(stage["moodFrom"], stage["moodTo"], progress * 0.6)
    bot = hexlerp("#1a1622", stage["moodTo"], progress)
    img = vgrad(VW, VH, top, bot)
    ...
    rrect(d, (bx + 2, by + 4, bx + 2 + bw, by + 4 + bh), 6, (0, 0, 0, 70))
    rrect(d, (bx, by, bx + bw, by + bh), 6, (44, 36, 52, 255))
